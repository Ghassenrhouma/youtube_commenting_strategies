"""Strategy 2, Account 1 — Observation: posts simple casual top-level comment, no DocShipper."""
import os
import time
import random
...
from comment_generator import generate_s2_observation
...
def run_session():
    target = s2_get_pending()

    if not target:
        seen_ids = s2_get_all_ids()
        new_videos = _find_new_videos(seen_ids)
        for v in new_videos:
            s2_add_target(v["video_id"], v["video_title"])
        target = s2_get_pending()

    if not target:
        print("[S2-A1] No pending targets found — will retry")
        return

    video_id = target["video_id"]
    video_title = target["video_title"]
    print(f"[S2-A1] Processing: {video_id} — {video_title[:60]}")

    comment = generate_s2_observation(video_title, "")
    print(f"[S2-A1] Generated comment: {comment[:100]}")
    ...
    print(f"[S2-A1] Done — status set to a_done")


def main():
    print(f"[S2-A1] Starting — DRY_RUN={DRY_RUN}, SKIP_DELAYS={SKIP_DELAYS}")
    while True:
        sleep_if_night()
        try:
            run_session()
        except Exception as e:
            print(f"[S2-A1] ERROR: {e}")
        gap = random.uniform(2700, 5400) if not SKIP_DELAYS else 10
        print(f"[S2-A1] Sleeping {gap:.0f}s before next session")
        time.sleep(gap)
